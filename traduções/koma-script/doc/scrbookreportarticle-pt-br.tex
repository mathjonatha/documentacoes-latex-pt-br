...
% ======================================================================
%
% Capítulo sobre scrbook, scrreprt e scrartcl do guia KOMA-Script
...
\chapter{As Classes Principais: \Class{scrbook}, \Class{scrreprt}, and
	\Class{scrartcl}}
\labelbase{maincls}%
\BeginIndexGroup
...
\Class{book}\IndexClass{book}, \Class{report}\IndexClass{report}, e
\Class{article}\IndexClass{article}. Também há um substituto para a
classe padrão \Class{letter}\IndexClass{letter}. A classe de documento para
cartas é descrita em um capítulo separado porque é fundamentalmente
diferente das três classes principais (veja \autoref{cha:scrlttr2}).

A forma mais simples de usar uma classe {\KOMAScript} em vez de uma padrão é
substituir o nome da classe no comando \verb|\documentclass| de acordo com \autoref{tab:\LabelBase.overview}. Por exemplo, você pode substituir
\Macro{documentclass}\PParameter{book} por
\Macro{documentclass}\PParameter{scrbook}. Normalmente, o {\LaTeX} deve processar
...
%  \caption
  \begin{captionbeside}
  [Correspondência de Classes]{\label{tab:\LabelBase.overview}Correspondência entre
  	classes padrão e classes {\KOMAScript}}
  [l]
  \begin{tabular}[t]{ll}
    \toprule
    classe padrão  & classe \KOMAScript{} \\%& \Script-Stil (\LaTeX2.09)\\
    \midrule
...
eliminando completamente o pacote problemático. Às vezes, o pacote
\href{https://www.ctan.org/pkg/scrhack}{\Package{scrhack}} documentado em
\cite{pkg:scrhack:manual} também pode ajudar. Substituir pacotes obsoletos por sucessores atuais
...
\DescRef{\LabelBase.cmd.addchap*}\IndexCmd{addchap*} documentado em
...
\iffalse%
  Note that in addition to the options documentado em this section, the
...
\else%
  In addition to the options documentado em this section, the
  ...
  \IndexCmd{RedeclareSectionCommand} documentado em
...
documentado em \autoref{sec:tocbasic.declarenewtoc},
