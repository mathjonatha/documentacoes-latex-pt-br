...
\subsection{Exemplo: Dimitris Kapeta}

You need in this exemplo to use \tkzname{mkpos=.2} with \tkzcname{tkzMarkAngle} because the measure of $ \widehat{CAM}$ is too small.
...
\subsection{Exemplo :  John Kitzmiller }
Prove that $\dfrac{AC}{CE}=\dfrac{BD}{DF}$.

Another interesting exemplo from John, you can see how to use some extra opções like\\\ \tkzname{decoration} and \tkzname{postaction}  from \TIKZ\ with \tkzname{tkz-euclide}.
...
\subsection{Exemplo 1: from Indonesia}
...
\subsection{Exemplo 2: from Indonesia}
...
drawing geometry constructions and explicaçãos. Underlying
