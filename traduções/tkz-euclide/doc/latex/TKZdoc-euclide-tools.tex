\section{Ferramentas diversas e ferramentas matemáticas}
...
This involves creating a segment on a given half-line of the same length as a given segment . It is in fact the definição of a point.
...
There's an opção in \TIKZ\  named \tkzname{veclen}. This opção
...
\begin{NewMacroBox}{tkzCalcLength}{\oarg{local opções}\parg{pt1,pt2}}%
You can store the result with the macro \tkzcname{tkzGetLength} for exemplo \tkzcname{tkzGetLength\{dAB\}} \\
...
\subsubsection{Simple exemplo} % (fold)
\label{ssub:simple_exemplo}
...
\end{tikzpicture}
\end{tkzexample}
% subsubsection simple_exemplo (end)
...
In this exemplo, the radical axis $(EF)$ has been drawn. A point $H$ has been chosen on $(EF)$ and the power of the point $H$ with respect to the circle of center $A$ has been calculated as well as $PS^2$. You can check that the power of $H$ with respect to the circle of center $C$ as well as $HS'^2, HT^2, HT'^2$ give the same result.  
