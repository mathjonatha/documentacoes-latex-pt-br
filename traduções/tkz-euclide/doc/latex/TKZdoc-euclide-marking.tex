\subsection{Marcar um segmento \tkzcname{tkzMarkSegment}}
\hypertarget{tms}{}

 \begin{NewMacroBox}{tkzMarkSegment}{\oarg{local opções}\parg{pt1,pt2}}%
A macro permite que você coloque uma marca em um segmento.
...
\midrule
\TOline{pos}{.5}{posição da marca}
\TOline{color}{black}{cor da marca}
\TOline{mark}{none}{escolha da marca}
\TOline{size}{4pt}{tamanho da marca}
\bottomrule
\end{tabular}

As marcas possíveis são aquelas fornecidas por \TIKZ, mas outras marcas foram criadas com base em uma ideia de Yves Combe.
\end{NewMacroBox} 

\subsubsection{Várias marcas}
...
\begin{NewMacroBox}{tkzMarkSegments}{\oarg{local opções}\parg{pt1,pt2 pt3,pt4 ...}}%
Argumentos are a list of pairs of points separated by spaces. The styles of \TIKZ\ are available for plots.
\end{NewMacroBox} 
...
\hypertarget{tms}{}  
 ...
 \begin{NewMacroBox}{tkzMarkArc}{\oarg{local opções}\parg{pt1,pt2,pt3}}% 
...
More delicate operation because there are many opções. The symbols used for marking in addition to those of \TIKZ\ are defined in the file |tkz-lib-marks.tex| and designated by the following characters:\begin{tkzltxexample}[]
|, ||,|||, z, s, x, o, oo 
\end{tkzltxexample}
...
\begin{NewMacroBox}{tkzMarkAngle}{\oarg{local opções}\parg{A,O,B}}%
$O$ is the vertex. Attention the argumentos vary according to the opções. Several markings are possible. You can simply draw an arc or  add a mark on this arc. The style of the arc is chosen with the opção \tkzname{arc}, the radius of the arc is given by \tkzname{mksize}, the arc can, of course, be colored.
...
\DeleteShortVerb{\|}
\subsubsection{Exemplo with \tkzname{mark = x} and with \tkzname{mark =||}}
...
\MakeShortVerb{\|}
\begin{NewMacroBox}{tkzMarkAngles}{\oarg{local opções}\parg{A,O,B}\parg{A',O',B'}etc.}%
With common opções, there is a macro for multiple angles.
  \end{NewMacroBox}  

\subsection{Problem to mark a small angle: {\tkzname{Opção veclen}}}\label{opt-veclen}
  ...
  In the next exemplo without the "scope" the result is :  Latex Error:  Dimension too large.
...
\begin{NewMacroBox}{tkzMarkRightAngle}{\oarg{local opções}\parg{A,O,B}}%
The \tkzname{german} opção allows you to change the style of the drawing. The opção \tkzname{size} allows to change the size of the drawing.
...
\subsubsection{Exemplo of marking a right angle} 
\begin{tkzexample}[latex=6cm,small]
\begin{tikzpicture}
...
\subsubsection{Exemplo of marking a right angle, german style} 
\begin{tkzexample}[latex=6cm,small]
\begin{tikzpicture}
  \tkzDefPoints{0/0/A,3/1/B,0.9/-1.2/P}
  \tkzDefPointBy[projection = onto B--A](P)  \tkzGetPoint{H}
...
\begin{NewMacroBox}{tkzMarkRightAngles}{\oarg{local opções}\parg{A,O,B}\parg{A',O',B'}etc.}%
With common opções, there is a macro for multiple angles.
\end{NewMacroBox}
...
\begin{NewMacroBox}{tkzPicAngle}{\oarg{tikz opções}\parg{A,O,B}}%
...
\midrule
\TOline{tikz opção}{see below}{drawing of the angle $\widehat{AOB}$.}
\end{tabular} 
\end{NewMacroBox}  

\begin{NewMacroBox}{tkzPicRightAngle}{\oarg{tikz opções}\parg{A,O,B}}%
...
opções             & exemplo & definição         \\ 
\midrule
\TOline{tikz opção}{see below}{drawing of the right angle $\widehat{AOB}$.}
\end{tabular} 

\medskip
\emph{You need to know possible opções of the \tkzname{angles} library}
\end{NewMacroBox} 
