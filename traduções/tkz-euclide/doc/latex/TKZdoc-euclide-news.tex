\section*{Novidades e compatibilidade}

\subsection{With 5.10 version}
\begin{itemize}
  \item Added french documentation
  
  \item Added the \code{mini} opção. You can use this opção with the \tkzNamePack{tkz-elements} package. Only the modules required for tracing will be loaded. This opção is currently only available if you are using \tkzNamePack{tkz-elements}.
\end{itemize}


% \subsection{With 5.06 version}
%      \begin{itemize}
%       \item Correction of a bug with the macro |\tkzLabelAngle| and the opção \code{angle}.
...
%
%  Correction of the documentation in Complete but minimal exemplo.
%
...
% \begin{itemize}
%
%     \item Finally, I added the \code{lua} opção for the package \tkzname{\tkznameofpack}. This allows to do the calculations for the main functions using lua; (see \ref{calc_with_lua}). The syntax is unchanged. Nothing changes for the user.
%
%   \item The \code{xfp} opção has become  \code{veclen} see \ref{opt-veclen}.
%
% \end{itemize}
%
...
%
% Some changes have been made to make the syntax more homogeneous and especially to distinguish the definição and search for coordinates from the rest, i.e. drawing, marking and labelling.
% Now the definição macros are isolated, it will be easier to introduce a phase of coordinate calculations using \tkzimp{Lua}.
%
...
%
%
% \item I recently discovered a problem when using the \code{scale} opção. When plotting certain figures with certain tools, extensive use of |pgfmathreciprocal| involves small computational errors but can add up and render the figures unfit. Here is how to proceed to avoid these problems:
% \begin{enumerate}
%
%   \item On my side I introduced a patch proposed by Muzimuzhi that modifies
% |pgfmathreciprocal|;
%
% \item  Another idea proposed by Muzimuzhi is to pass as an opção for the |tikzpicture| environment this |/pgf/fpu/install only={reciprocal}| after loading of course the |fpu| library;
%
...
%
% \item  In order to isolate the definiçãos, I deleted or modified certain macros which are: |\tkzDrawLine|, |\tkzDrawTriangle|, |\tkzDrawCircle|, |\tkzDrawSemiCircle| and  |\tkzDrawRectangle|;
%
...
%
%
% \item I added the macro |\tkzPicAngle[tikz opções](A,B,C)| for those who prefer to use  \TIKZ ;
%
%
% \item The macro |\tkzMarkAngle| has been corrected;
%
% \item The macro linked to the \tkzname{apollonius} opção of the |\tkzDefCircle| command has been rewritten;
%
% \item (4.23) The macro |\tkzDrawSemiCircle| has been corrected;
%
% \item
% The order of the argumentos of the macro \tkzcname{tkzDefPointOnCircle} has changed: now it is center, angle and point or radius.
% I have added two opções for working with radians which are \tkzname{through in rad} and \tkzname{R in rad}.
%
%
% \item I added the opção \tkzname{reverse} to the arcs paths. This allows to reverse the path and to reverse if necessary the arrows that would be present.
%
%
...
%
% \item Correction opção \tkzname{return} witk \tkzcname{tkzProtractor}
%
...
%
%
% \item The circle inversion was badly defined so I rewrote the macro. The input argumentos are always the center and a point of the circle, the output argumentos are the center of the image circle and a point of the image circle or two points of the image line if the antecedent circle passes through the pole of the inversion. If the circle passes the inversion center, the image is a straight line, the validity of the procedure depends on the choice of the point on the antecedent circle;
%
% \item Correct allocation for gold sublime and euclide triangles;
%
%
% \item I added the opção \code{next to} for the intersections LC and CC;
%
%
% \item Correction opção isoceles right;
%
...
%
%
% \item The opções \code{orthogonal from} and \code{through} depend now of \tkzcname{tkzDefCircleBy}
%
% \begin{enumerate}
%
...
%
%
% \item Now it's possible to add an arrow on a line or a circle with the opção |tkz arrow|.
% \end{enumerate}
% \end{itemize}
%
...
%
% \item  Logically most macros accept \TIKZ\ opções. So I removed the \code{duplicate} opções when possible thus the \code{label opções} opção is removed;
%
...
%
% \item Now |\tkzClipCircle| and |\tkzClipPolygon| have an opção \tkzimp{out}. To use this opção you must have a Bounding Box that contains the object on which the Clip action will be performed. This can be done by using an object that encompasses the figure or by using the macro \tkzcname{tkzInit};
%
%
% \item The opções \tkzname{end} and \tkzname{start} which allowed to give a label to a straight  line are removed. You now have to use the macro \tkzcname{tkzLabelLine};
%
...
%
% \item  The notion of vector disappears, to draw a vector just pass "->" as an opção to \tkzcname{tkzDrawSegment};
%
%
...
%
%
% \item An opção of the macro \tkzcname{tkzDefTriangle} has changed, in the previous version the opção was \code{euclide} with an \code{e}. Now it's \code{euclid};
%
...
%
%
% \item Opção \code{isosceles right} is a new opção of the macro \tkzcname{tkzDefTriangle};
%
