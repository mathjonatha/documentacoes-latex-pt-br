\section{Alguns exemplos interessantes}
...
\end{tkzexample}
 ...
 \begin{minipage}{0.90\textwidth}
   { \emph{To inscribe a pentagon in a given square.
 }} 
 \end{minipage}
 };
 \end{tikzpicture}%
...
\node [mybox,title={Tangent lines to two circles with radical axis}] (box){%
\begin{minipage}{0.90\textwidth}
  {\emph{This exemplo involves determining the middle of a segment, using only a compass.}}
\end{minipage}
};
\end{tikzpicture}%
...
\subsection{Definição of a circle  \_Apollonius\_}

\begin{tikzpicture}
\node [mybox,title={Definição of a circle  \_Apollonius\_}] (box){%
\begin{minipage}{0.90\textwidth}
...
A circle is the set of points in a plane that are equidistant from a given point O. The distance r from the center is called the radius, and the point O is called the center. It is the simplest definição but it is not the only one. Apollonius of Perga gives another definição :
The set of all points whose distances from two fixed points are in a constant ratio is a circle.

With \pkg{tkz-euclide} is easy to show you the last definição
...
The purpose of the first  exemplos was to show the simplicity with which we could recreate these propositions. With TikZ you need to do calculations and use trigonometry while with \pkg{tkz-euclide} you only need to build simple objects
...
The last exemplo is very complex and it is to show you all that we can do with \pkg{tkz-euclide}.
